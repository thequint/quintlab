...
function weather_timer()
{
	for(i=0;i<city_array.length;i++)
		{
			// get city id by name city_array[i]
			get_city_id_by_name(city_array[i])
		}
}
function get_city_id_by_name(name)
{
	var  city_id = // copy getIdByCityName(city_name) code 
	get_weather(city_id)
}

function get_weather(city_id)
{
	console.log(weather for this city current)// accuweather API for current conditions
		console.log(min and max)// accuweather API for forecast
}
